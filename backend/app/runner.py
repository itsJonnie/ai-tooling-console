...
from app.models import RunState, LogEntry, MetricPoint
...
async def simulate_training(run_id: str) -> None:
    ...
    failure_epoch = None
    if should_fail and run.epochs >= 1:
        # Fail at a random epoch (early to mid training)
        failure_epoch = random.randint(1, max(1, min(run.epochs, 5)))
    ...
    run = storage.get_run(run_id)
    if run and run.state == RunState.RUNNING:
